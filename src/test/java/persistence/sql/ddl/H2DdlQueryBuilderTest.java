...
class H2DdlQueryBuilderTest {
	...
	void When_DbTypeIsH2_Then_GetH2Diaalect() {
		assertEquals(ddlQueryBuilder.dialect.getClass(), H2Dialect.class);
	}

	@DisplayName("Person 객체로 CREATE 쿼리 생성 테스트")
	@Test
	void test_createQuery() {
		assertEquals(ddlQueryBuilder.createQuery(entityMetadata),
	...
	@Test
	void test_dropQuery() {
		assertEquals(ddlQueryBuilder.dropQuery(entityMetadata),
